...
$$
W(\mu,\nu) := \inf_{\pi \in \Pi(\mu,\nu)} \biggl(\int_{X \times Y} \|x-y\|_2^2 \mathrm{d}x \pi(x,y) \biggr)^{\frac{1}{2}}.
$$
...
![](figs/camels_2d_wasserstein.png)
...
![Some other example shapes from Mesh Data from Deformation Transfer for Triangle Meshes (Summer, Popovic)](figs/animal_collection.png)
...
```{python}
#| fig-cap: Computing GW distances and embedding via Multidimensional Scaling
...
px.scatter_3d(x=pos[:, 0], y=pos[:, 1], z=pos[:, 2], color = kmeans.labels_, hover_name=names)
```
